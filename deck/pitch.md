...
Intel silicon via OpenVINO. AI Infra Summit Hackathon — Intel Online track.
...
- Designated-arm task table: instruction determines arm + object + bin
- Honest kinematic-carry data-gen shortcut, documented

## Slide 5 — VLA
- ACT (52M) + `observation.environment_state` language slot
- Frozen MiniLM embeddings => rephrased commands work (paraphrase 4/4)
- Instruction-swap video: wrong instruction reroutes the correct arm

## Slide 6 — Intel deployment (the required slide)
- ONNX -> OpenVINO IR (normalize+policy+denormalize all in-graph)
- i9-13900K CPU: 28.9 Hz policy rate — 3.2x PyTorch CPU
- UHD 770 iGPU: 28.6 Hz — real iGPU execution, not a CPU fallback
...
- Typed-command fallback; honest decline on unparseable commands

## Slide 8 — Reproducibility & Limits
...
- Limits: grasp-assist abstraction at eval, 3-task scope, ~50% success
  ceiling at 14k steps (more data/steps -> higher)
